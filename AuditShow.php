...
<head>
	...
	<link rel="stylesheet" href="AuditShow.css">
</head>
...
<body>
	<div class="container1">
		...
		$sql = "SELECT * FROM audit  WHERE (StudentID LIKE '%" . $_GET["txtKeyword"] . "%' or BikeID LIKE '%" . $_GET["txtKeyword"] . "%' )  LIMIT $offset, $no_of_records_per_page ";
		...
		while ($row = mysqli_fetch_array($res_data) and $rowstudent = mysqli_fetch_array($res_student)) {
			echo "<tr>
					...
					<td>" . $row["Status"] . "</td>
					<td><a href=ReturnBikeFrom.php?BikeID=" . $row["BikeID"] . ">ReturnBike</a> </td>
